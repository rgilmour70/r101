...
const Navigation = (props) => {
	...
	const { numberOfSlides, slideNumber, onNavEvent, navFrozen } = props;
	...
	const leftNavButton = <button className="btn btn-primary" onClick={ () => onNavEvent(slideNumber-1, numberOfSlides) } >{leftArrow}</button>

	let rightNavButton = "";
	if (navFrozen) {
		rightNavButton = <button className="btn btn-primary frozen">{rightArrow}</button>
	} else {
		rightNavButton = <button className="btn btn-primary" onClick={ () => onNavEvent(slideNumber+1, numberOfSlides) } >{rightArrow}</button>
	}
